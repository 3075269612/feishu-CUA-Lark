...
CUA-Lark 是一个面向飞书 / Lark 桌面端的视觉优先 CUA（Computer-Use Agent）智能测试代理工程骨架。当前阶段已完成 Phase 0–6：IM 收发、Calendar CRUD、Docs 创建与编辑、跨产品联动、自然语言→TaskSpec 转换，全部通过真实飞书桌面端验证。
...
- YAML `TaskSpec` 加载 + **自然语言→TaskSpec 转换**（`--nl` 参数）。
...
- Phase 4 Calendar：6 阶段 `CalendarCreateSkill` 状态机，创建日程通过真实飞书验证。
- Phase 5 Docs：`DocsCreateSkill` 状态机创建空白文档（GUI 点击链路）、编辑已有文档（Ctrl+F 键盘流定位）。
- **Phase 6 Cross-Product**：`CrossProductSkill` 元状态机串联 Calendar→Docs→IM 汇聚，全部通过真实飞书验证。
...
# Cross-Product: 跨产品联动
D:/2026上/feishu-CUA-Lark/.venv/Scripts/python.exe -m cua_lark.main run testcases/cross_product/kickoff_flow.yaml --mock

# === 自然语言输入（--nl）===
# 发送 IM 消息
D:/2026上/feishu-CUA-Lark/.venv/Scripts/python.exe -m cua_lark.main run --nl "在CUA-Lark-Test群发一条消息：测试" --mock

# 创建日历日程
D:/2026上/feishu-CUA-Lark/.venv/Scripts/python.exe -m cua_lark.main run --nl "在日历中创建CUA-Lark项目启动会议，明天下午3点" --mock

# 创建文档
D:/2026上/feishu-CUA-Lark/.venv/Scripts/python.exe -m cua_lark.main run --nl "在CUA-Lark-Test-Docs文件夹创建CUA测试文档" --mock

# 自然语言 + 真实 UI 发送
D:/2026上/feishu-CUA-Lark/.venv/Scripts/python.exe -m cua_lark.main run --nl "在CUA-Lark-Test群发一条消息：端到端测试" --real-ui --confirm-target CUA-Lark-Test --allow-send
```
...
7. Phase 6：跨产品链路 + NL→TaskSpec 自然语言解析。✅
