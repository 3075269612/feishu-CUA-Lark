...
CUA-Lark 是一个面向飞书 / Lark 桌面端的视觉优先 CUA（Computer-Use Agent）智能测试代理工程骨架。当前阶段已完成 Phase 0 / Phase 1，并进入 Phase 2A：在严格安全门禁下支持 IM 真实桌面 dry-run 和可选真实发送。
...
- 当前阶段没有 OCR/VLM/API oracle，真实发送后的报告只作为截图证据和人工复核依据，不标记强自动 pass。
