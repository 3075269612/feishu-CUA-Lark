...
Phase 2B 不接入真实 VLM、OCR、Accessibility Tree、飞书 OpenAPI、MCP、OpenCLI、GUI-R1 或 ScaleCUA。API/OCR/VLM verifier 目前是可插拔占位，未配置时返回 skipped。固定坐标方案仍只是临时真实 UI 验收路径，不是最终智能定位方案。
